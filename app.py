    ...
    if pag == "📤 Upload":
        ...
        with tab_nf:
            ...
            with c2:
                ...
                status_nf = st.text_input("Status", placeholder="Ex: Em separação, Saiu para entrega, Entregue...")
                obs_nf    = st.text_input("Data prevista de entrega", placeholder="Ex: 20/03/2026")
                st.divider()
        ...
        if nfs:
            ...
            for row in nfs_f:
                with st.expander(f"NF {row['numero_nf']} — {row['cliente']} — {formatar_valor(row['valor'])}"):
                    ...
                    nova_obs = c5.text_input("Data prevista de entrega", value=row.get("observacao") or "", key=f"ob_{row['id']}")
