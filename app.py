...
Uma URL só. O login identifica quem é admin ou cliente.

Admin:   usa CNPJ especial "admin" + senha master
Cliente: usa CNPJ da empresa + senha cadastrada
"""
...
def badge(status: str) -> str:
    mapa = {
        ...
        "cancelado": ("#f3f4f6", "#374151", "Cancelado"),
    }
...
def botao_pdf(pdf_base64: str, nome_arquivo: str, label: str):
    if not pdf_base64:
...
def tela_login():
    ...
    with col_form:
        ...
        if entrar:
            ...
            if cnpj.strip().lower() == "admin" and senha == admin_senha:
                st.session_state["perfil"] = "admin"
                st.session_state["usuario"] = {"nome": "Administrador"}
                st.rerun()
                return

            # ── Verifica se é cliente ──
            cliente = db.buscar_cliente_cnpj(cnpj.strip())
            if not cliente:
                st.error("CNPJ não encontrado.")
                return
            if hash_senha(senha) != cliente["senha_hash"]:
        ...
        st.markdown("""
            <p style="text-align:center;font-size:12px;color:#9ca3af;margin-top:1rem">
                Primeiro acesso? Solicite suas credenciais ao escritório.
            </p>
        """, unsafe_allow_html=True)
...
def interface_admin():
    ...
    if pagina == "📤 Upload Documentos":
        ...
        with tab_nf:
            ...
            with col1:
                ...
                arq = st.file_uploader("Selecione o arquivo", type=["pdf"], key="up_nf")
                if arq:
                    byt = arq.read()
            ...
            with col2:
                ...
                if st.button("💾 Salvar NF", type="primary", use_container_width=True):
                    ...
                    if not byt:
                        st.error("Faça upload do PDF primeiro.")
                    elif not numero_nf:
        ...
        with tab_boleto:
            ...
            with col1:
                ...
                arq_b = st.file_uploader("Selecione o arquivo", type=["pdf"], key="up_bol")
                if arq_b:
                    byt_b = arq_b.read()
            ...
            with col2:
                ...
                if st.button("💾 Salvar Título", type="primary", use_container_width=True):
                    ...
                    if not byt_b:
                        st.error("Faça upload do PDF primeiro.")
                    elif not numero_titulo or not vencimento:
                        st.error("Preencha número e vencimento.")
                    else:
                        db.inserir_titulo(opcoes[cli_sel_b], numero_titulo, valor_bol, vencimento,
                                          pdf_para_base64(byt_b), st.session_state.get("bol_n","boleto.pdf"),
                                          op_nf[nf_vinc])
                        st.success(f"✅ Título {numero_titulo} salvo!")
                        for k in ["bol_d","bol_b","bol_n"]:
                            st.session_state.pop(k, None)
                        st.rerun()
    ...
    # ── Clientes ──────────────────────────────────────────────────────────
        ...
        with st.expander("➕ Novo cliente"):
            with st.form("form_cli"):
                ...
                with c1:
                    ...
                    cnpj_c  = st.text_input("CNPJ *")
                with c2:
                    email_c = st.text_input("E-mail")
                    wpp_c   = st.text_input("WhatsApp")
                senha_c = st.text_input("Senha de acesso *", type="password")
                if st.form_submit_button("Cadastrar", type="primary"):
                    if not nome_c or not cnpj_c or not senha_c:
        ...
        if clientes:
            st.dataframe(
                [{"Nome": c["nome"], "CNPJ": c["cnpj"], "Email": c["email"] or "—",
    ...
    # ── Notas Fiscais ──────────────────────────────────────────────────────
        ...
        if nfs:
            c1, c2 = st.columns(2)
            with c1:
                filtro_cli = st.selectbox("Cliente", ["Todos"] + list({n["cliente"] for n in nfs}))
            with c2:
                filtro_st  = st.selectbox("Status", ["Todos", "ativo", "cancelado"])

            nfs_f = [n for n in nfs
                     if (filtro_cli == "Todos" or n["cliente"] == filtro_cli)
                     and (filtro_st == "Todos" or n["status"] == filtro_st)]

            st.subheader("Editar rastreio")
            for row in nfs_f:
                with st.expander(f"NF {row['numero_nf']} — {row['cliente']} — {formatar_valor(row['valor'])}"):
                    c1, c2, c3 = st.columns([2, 2, 1])
                    cod   = c1.text_input("Código rastreio", value=row["codigo_rastreio"] or "", key=f"c_{row['id']}")
                    tra   = c2.text_input("Transportadora",  value=row["transportadora"]  or "", key=f"t_{row['id']}")
                    c3.write(""); c3.write("")
                    if c3.button("Salvar", key=f"s_{row['id']}"):
                        db.atualizar_rastreio(row["id"], cod, tra)
                        st.success("Salvo!"); st.rerun()

            st.divider()
            st.dataframe(
                [{"Cliente": n["cliente"], "NF": n["numero_nf"],
                  "Valor": formatar_valor(n["valor"]), "Emissão": n["data_emissao"],
                  "Status": n["status"], "Transportadora": n["transportadora"] or "—"}
                 for n in nfs_f],
                use_container_width=True, hide_index=True,
            )
        else:
    ...
    # ── Títulos ────────────────────────────────────────────────────────────
        ...
        if titulos:
            ...
            for t in em_aberto:
                ...
                if c4.button("✅ Pago", key=f"p_{t['id']}"):
                    db.marcar_titulo_pago(t["id"])
                    st.success("Pago!"); st.rerun()
            st.divider()
            st.dataframe(
                [{"Cliente": t["cliente"], "Título": t["numero_titulo"],
                  "Valor": formatar_valor(t["valor"]),
                  "Vencimento": t["vencimento"], "Status": t["status"]}
                 for t in titulos],
                use_container_width=True, hide_index=True,
            )
        else:
        ...
        if not nfs:
            ...
            for nf in nfs:
                with st.container(border=True):
                    ...
                    c3.markdown(badge("ativo"), unsafe_allow_html=True)

                    if nf.get("codigo_rastreio"):
                        st.caption(f"📦 {nf['transportadora'] or 'Transportadora'} · `{nf['codigo_rastreio']}`")
            ...
            for nf in nfs:
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        st.caption(f"Emitida em {nf['data_emissao']}")
                    c2.markdown(badge("ativo"), unsafe_allow_html=True)
                    st.code(nf["codigo_rastreio"], language=None)

                    transp = (nf.get("transportadora") or "").lower()
                    cod    = nf["codigo_rastreio"]
                    if "correios" in transp:
                        st.link_button("🔗 Rastrear nos Correios",
                                       "https://rastreamento.correios.com.br",
                                       use_container_width=True)
                    elif "jadlog" in transp:
                        st.link_button("🔗 Rastrear na Jadlog",
                                       f"https://www.jadlog.com.br/jadlog/tracking.jad?cte={cod}",
                                       use_container_width=True)
                    else:
                        st.link_button("🔍 Buscar rastreio",
                                       f"https://www.google.com/search?q=rastrear+{cod}",
                                       use_container_width=True)
        ...
        if not titulos:
            ...
            for t in titulos:
                ...
                with st.container(border=True):
                    ...
                    c3.markdown(badge(status_vis), unsafe_allow_html=True)
...
if not perfil:
    ...
    interface_cliente(st.session_state["usuario"])
